...
function showMap(meta) {
	...
	console.log(
		META_KEY_BASEMAP,
		meta[META_KEY_BASEMAP],
		BASEMAP_LIST[meta[META_KEY_BASEMAP]],
	);
	if (
		meta[META_KEY_WMS_URL] !== undefined &&
		meta[META_KEY_WMS_LAYERS] !== undefined
	) {
		L.tileLayer
			.wms(meta[META_KEY_WMS_URL], {
				layers: meta[META_KEY_WMS_LAYERS],
			})
			.addTo(map);
	} else if (
		meta[META_KEY_BASEMAP] !== undefined &&
		BASEMAP_LIST[meta[META_KEY_BASEMAP]] !== undefined
	) {
		L.tileLayer(BASEMAP_LIST[meta[META_KEY_BASEMAP]], {
...
function renderData(map, data, meta) {
	...
	const iconMarker = meta[META_KEY_ICON]
		? L.ExtraMarkers.icon({
	...
	function showMarker(latLon, tags) {
		const marker = iconMarker
			? L.marker(latLon, { icon: iconMarker })
			: L.marker(latLon);
...
function parseData() {
	...
	for (const [key, value] of new URLSearchParams(location.search).entries()) {
		if (key.startsWith(NOT_PREFIX)) {
			notTags[removePrefix(key, NOT_PREFIX)] = value;
	...
	if (
		meta[META_KEY_WMS] !== undefined &&
		WMS_LIST[meta[META_KEY_WMS]] !== undefined
	) {
		meta[META_KEY_WMS_URL] = WMS_LIST[meta[META_KEY_WMS]];
	}
	return { tags, notTags, meta };
}
...
function buildOverpassQuery(tags, notTags, meta) {
	const tagsSelector = Object.entries(tags)
		.map(([tag, value]) =>
			value === "" ? `["${tag}"]` : `["${tag}"="${value}"]`,
		)
		.join("");
	const notTagsSelector = Object.entries(notTags)
		.map(([tag, value]) =>
			value === "" ? `[!"${tag}"]` : `["${tag}"!="${value}"]`,
		)
		.join("");
	return `
        ...
        (
          ${meta[META_KEY_TYPE]}${tagsSelector}${notTagsSelector}(area.searchArea);
        );
        out body;
        >;
        out skel qt;
...
function addTagKeyInput() {
	...
	lastValueElement.insertAdjacentElement(
		"afterend",
